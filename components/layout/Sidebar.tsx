...
const Sidebar: React.FC = () => {
  ...
  return (
    <>
      ...
      />
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-72 max-w-[90vw] flex-col border-r border-slate-200 bg-[var(--color-surface)] shadow-xl transition-transform duration-300 ease-in-out dark:border-slate-800 dark:bg-gray-900 md:static md:z-0 md:flex md:max-w-none md:translate-x-0 md:shadow-none md:w-64 md:shrink-0 ${
            ...
            <AnimatePresence>
              {!isSidebarCollapsed && (
                <motion.div
                  ...
                  className="hidden text-left text-lg font-black leading-tight text-[var(--color-text-primary)] md:block"
                >
                  <span className="block">IEE 6049</span>
                  <span className="block">Ricardo Palma</span>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
